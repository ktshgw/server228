(() => {
    ...
    function closeUpdateMenu() {
        $("#update-menu-popover").hidden = true;
        $("#update-menu-button").setAttribute("aria-expanded", "false");
    }

    function toggleUpdateMenu() {
        const popover = $("#update-menu-popover");
        popover.hidden = !popover.hidden;
        $("#update-menu-button").setAttribute("aria-expanded", String(!popover.hidden));
    }

    function closeChangelogVersionMenu() {
        const menu = $("#changelog-version-menu");
        if (!menu) return;
        menu.hidden = true;
        $("#changelog-version-button").setAttribute("aria-expanded", "false");
    }

    function toggleChangelogVersionMenu() {
        const menu = $("#changelog-version-menu");
        menu.hidden = !menu.hidden;
        $("#changelog-version-button").setAttribute("aria-expanded", String(!menu.hidden));
    }
    ...
    function parseRoute() {
        ...
        return {
            ...
            release: route === "changelog" && /^[0-9]{1,2}\.[0-9]{1,2}\.[0-9]{2}[bs][0-9]*$/.test(rawId || "") ? rawId : null,
        };
    }
    ...
    async function route() {
        ...
        if (parsed.route === "changelog") {
            if (previousRoute !== "changelog") {
                state.changelog.showHotfixes = true;
                if (!parsed.release) state.changelog.selected = null;
            }
            if (parsed.release) state.changelog.selected = parsed.release;
        }
    ...
    async function loadChangelogData() {
        ...
        state.changelog.announcements = Array.isArray(payload.announcements) ? payload.announcements : [];
    }

    function renderHomeNews() {
        const releases = (state.changelog.releases || []).map((release, index) => ({
            ...release,
            newsType: "release",
            sourceOrder: index,
        }));
        const announcements = (state.changelog.announcements || []).map((announcement, index) => ({
            ...announcement,
            newsType: "announcement",
            sourceOrder: releases.length + index,
        }));
        const news = [...releases, ...announcements].sort((left, right) =>
            String(right.date || "").localeCompare(String(left.date || "")) || left.sourceOrder - right.sourceOrder,
        );
        const cards = news.slice(0, 4).map((item) => {
            const isRelease = item.newsType === "release";
            const card = make(isRelease ? "a" : "article", `news-card update-news-card is-${item.type || item.newsType}`);
            if (isRelease) card.href = `#changelog/${item.version}`;

            const art = make("div", "news-art news-art-update");
            art.append(make("span", "", isRelease
                ? `${item.type === "hotfix" ? "HOTFIX" : "RELEASE"} ${item.version}`
                : (item.label || "SOMS! NEWS")));

            const date = new Date(`${item.date}T00:00:00Z`);
            const time = make("time", "", Number.isNaN(date.getTime()) ? "" : new Intl.DateTimeFormat("ru-RU", {
                day: "numeric", month: "short", year: "numeric", timeZone: "UTC",
            }).format(date));
            const body = make("div");
            body.append(make("h3", "", isRelease
                ? `${item.type === "hotfix" ? "Hotfix" : "Release"} ${item.version}`
                : item.title));
            if (isRelease) {
                const list = make("ul");
                const changes = (item.groups || []).flatMap((group) => {
                    const firstItem = (group.items || [])[0];
                    return firstItem ? [{ text: firstItem, kind: group.kind }] : [];
                }).slice(0, 4);
                list.append(...changes.map((change) => {
                    const row = make("li", "", change.text);
                    row.dataset.kind = change.kind;
                    return row;
                }));
                body.append(list, make("strong", "update-read-more", "Full changelog ›"));
            } else {
                body.append(make("p", "", item.summary || ""));
            }
            const copy = make("div", "news-copy");
            copy.append(time, body);
            card.append(art, copy);
            return card;
        });
        $("#home-news").replaceChildren(...cards);
        $("#home-news-more").hidden = news.length <= 4;
    }

    function visibleChangelogReleases() {
        return (state.changelog.releases || []).filter(
            (release) => state.changelog.showHotfixes || release.type !== "hotfix",
        );
    }

    function moveChangelogRelease(offset) {
        const releases = visibleChangelogReleases();
        const index = releases.findIndex((release) => release.version === state.changelog.selected);
        const target = releases[index + offset];
        if (target) location.hash = `changelog/${target.version}`;
    }

    async function loadChangelog() {
        await loadChangelogData();
        const releases = visibleChangelogReleases();
        const selected = releases.find((release) => release.version === state.changelog.selected) || releases[0];
        if (!selected) throw new Error("История обновлений пока пуста");
        state.changelog.selected = selected.version;
        $("#changelog-show-hotfixes").checked = state.changelog.showHotfixes;
        if (parseRoute().release !== selected.version) history.replaceState(null, "", `#changelog/${selected.version}`);

        const currentButton = $("#changelog-version-button");
        currentButton.dataset.releaseType = selected.type || "release";
        $("#changelog-version-channel").textContent = selected.channel;
        $("#changelog-version-current").textContent = selected.version;
        $("#changelog-version-menu").replaceChildren(...releases.map((release) => {
            const active = release.version === selected.version;
            const button = make("button", `changelog-version-option${active ? " is-active" : ""}`);
            button.type = "button";
            button.dataset.releaseType = release.type || "release";
            button.setAttribute("role", "option");
            button.setAttribute("aria-selected", String(active));
            button.append(
                make("strong", "changelog-version-option-name", release.version),
                make("small", "", release.channel),
            );
            button.addEventListener("click", () => {
                closeChangelogVersionMenu();
                location.hash = `changelog/${release.version}`;
            });
            return button;
        }));
        const selectedIndex = releases.indexOf(selected);
        $("#changelog-newer").disabled = selectedIndex <= 0;
        $("#changelog-older").disabled = selectedIndex < 0 || selectedIndex >= releases.length - 1;

        const date = new Date(`${selected.date}T00:00:00Z`);
        $("#changelog-date").textContent = new Intl.DateTimeFormat("en-GB", {
            day: "numeric", month: "long", year: "numeric", timeZone: "UTC",
        }).format(date);
        $(".changelog-entry").dataset.releaseType = selected.type || "release";
        $("#changelog-channel").textContent = selected.channel;
        $("#changelog-version").textContent = selected.version;
        $("#changelog-summary").textContent = "";
        $("#changelog-groups").replaceChildren(...selected.groups.map((group) => {
            const section = make("section", "changelog-group");
            section.dataset.kind = group.kind;
            const list = make("ul");
            list.append(...group.items.map((item) => make("li", "", item)));
            section.append(make("h3", "", group.title), list);
            return section;
        }));
    }
    ...
    function bindEvents() {
        ...
        $("#update-menu-button").addEventListener("click", toggleUpdateMenu);
        $("#update-menu-popover").addEventListener("click", (event) => {
            if (event.target.closest("a")) closeUpdateMenu();
        });
        $("#changelog-show-hotfixes").addEventListener("change", async (event) => {
            state.changelog.showHotfixes = event.currentTarget.checked;
            closeChangelogVersionMenu();
            try {
                await busy(loadChangelog());
            } catch (error) {
                toast(error.message || "Не удалось обновить фильтр changelog", "error");
            }
        });
        $("#changelog-version-button").addEventListener("click", toggleChangelogVersionMenu);
        $("#changelog-newer").addEventListener("click", () => moveChangelogRelease(-1));
        $("#changelog-older").addEventListener("click", () => moveChangelogRelease(1));
...
$("#support-soms").addEventListener("click", () => {
    toast("Work in progress", "error");
});
        $("#home-news-more").addEventListener("click", () => {
            toast("Work in progress", "error");
        });
        ...
        document.addEventListener("pointerdown", (event) => {
            ...
            if (!$("#update-menu-popover").hidden && !event.target.closest(".update-menu-shell")) closeUpdateMenu();
            if (!$("#changelog-version-menu").hidden && !event.target.closest(".changelog-release-picker")) closeChangelogVersionMenu();
        });
