...
from app.service.soms_activity_service import (
    announcement_ready,
    chat_link,
    eligible_recipient,
...
class ActivityTests(unittest.IsolatedAsyncioTestCase):
    async def test_multiplayer_intermediate_winner_is_replaced_and_original_holder_preserved(self):
        for previous_id in (None, 9, 7):
            ...
            with (
                patch(
                    "app.service.soms_activity_service.current_winner", new=AsyncMock(return_value=(15, 7, "winner"))
                ),
                patch("app.service.soms_activity_service.eligible_recipient", new=AsyncMock(return_value=previous_id)),
            ):
    ...
    async def test_new_first_place_announced_with_previous_holder_at_end(self):
        db = self.session()
        with (
            patch("app.service.soms_activity_service.current_winner", new=AsyncMock(return_value=(15, 7, "winner"))),
            patch("app.service.soms_activity_service.eligible_recipient", new=AsyncMock(return_value=9)),
        ):
    ...
    async def test_own_improvement_nonwinner_and_retry_do_not_announce(self):
        for winner, previous, duplicate in [
            ...
            with patch("app.service.soms_activity_service.current_winner", new=AsyncMock(return_value=winner)):
    ...
    async def test_first_score_has_announcement_without_lost_notification(self):
        db = self.session()
        with patch("app.service.soms_activity_service.current_winner", new=AsyncMock(return_value=(15, 7, "winner"))):
    ...
    async def test_friend_outbox_is_unique_per_relationship(self):
        db = self.session()
        with patch("app.service.soms_activity_service.eligible_recipient", new=AsyncMock(return_value=9)):
    ...
    async def test_native_unfriend_notifies_but_unblock_does_not(self):
        ...
        for kind, path in [(RelationshipType.FOLLOW, "/friends/9"), (RelationshipType.BLOCK, "/blocks/9")]:
            ...
            with (
                patch("app.service.soms_activity_service.stage_friend_notification", new=AsyncMock()) as notify,
    ...
    async def test_new_map_poll_skips_history_and_deduplicates(self):
        from app.tasks.soms_announcements import announce_maps_job
        ...
        with (
            patch("app.tasks.soms_announcements.with_db", return_value=context),
    ...
    async def test_outbox_retry_reuses_persisted_message_id_and_populates_chat_cache(self):
        from app.service.soms_activity_service import deliver_announcements
        ...
        with (
            ...
            patch(
                "app.service.soms_activity_service.ensure_announce_channel",
