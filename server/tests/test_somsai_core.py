...
from app.database.somsai import (
    SomsaiLock,
    SomsaiMatch,
    SomsaiNativeRoom,
    SomsaiPartyInvite,
...
from app.models.somsai_admin import SomsaiPoolSpec
from app.router import lio, somsai_lio
from app.router.somsai_lio import router as interop_router
from app.service.somsai_match_service import (
    finish_match,
    match_action,
    members_of,
    room_event,
...
from app.service.somsai_party_service import (
...
from app.service.somsai_pool_service import save_pool
from app.service.somsai_rating_service import ensure_rating, initial_rating
from app.service.somsai_service import create_custom, join_custom, join_queue, leave_queue, public_state
...
class SomsaiCoreTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_native_room_claim_renews_somsai_room_and_new_lobby_leaves_match(self):
        ...
        with patch("app.router.somsai_lio.somsai_transaction", transaction):
    ...
    async def test_signed_http_cannot_reserve_disabled_ranked_even_with_valid_signature(self):
        ...
        request_id = str(uuid4())
        with (
            patch("app.dependencies.somsai_interop.settings.shared_interop_secret", "fixture-secret"),
            patch("app.router.somsai_lio.somsai_transaction", transaction),
        ):
            async with httpx.AsyncClient(transport=httpx.ASGITransport(app=app), base_url="http://test") as client:
                for _ in range(2):
...
class InteropSecurityTests(unittest.IsolatedAsyncioTestCase):
    async def test_missing_forged_and_expired_signatures_rejected_before_db(self):
        app = FastAPI()
        app.include_router(interop_router)
        with patch("app.dependencies.somsai_interop.settings.shared_interop_secret", "fixture-secret"):
            async with httpx.AsyncClient(transport=httpx.ASGITransport(app=app), base_url="http://test") as client:
