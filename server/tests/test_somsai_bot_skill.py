...
from app.service.somsai_bot_service import (
    _load_skill,
    _official_request,
    _persona_candidates,
    _upstream_budget,
...
from app.service.somsai_bot_skill import (
    choose_draft_slot,
    comfort_for,
    history_candidates,
    map_features,
...
from app.service.somsai_bot_tech import technical_features
...
class SkillTests(unittest.TestCase):
    def test_technical_patterns_and_mods_are_independent_of_slot_labels(self):
        fixture = (
            ...
            / ".sources/osu-server-spectator/osu.Server.Spectator.Tests/Multiplayer/Fixtures/bot-tech.osu"
        )
...
class HistoryTests(unittest.IsolatedAsyncioTestCase):
    async def test_top1000_all_ranks_and_pages_remain_available_with_warm_cache(self):
        ...
        found = set()
        with patch("app.service.somsai_bot_service._official_request", new=AsyncMock(side_effect=ranking)) as upstream:
            for rank in range(1, 1000, 50):
                with patch("app.service.somsai_bot_service.random.randint", return_value=rank):
        ...
        with patch("app.service.somsai_bot_service.random.randint", return_value=999):
            del redis.data["somsai:bot-personas:v3:osu:top1000:20"]
            with patch("app.service.somsai_bot_service._official_request", new=AsyncMock(side_effect=TimeoutError)):
    ...
    async def test_draft_enrichment_happens_outside_lock_and_preserves_revision(self):
        ...
        with (
            patch("app.service.somsai_party_service.somsai_transaction", transaction),
