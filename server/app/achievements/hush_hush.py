...
async def deliberation(
    ...
    eligible_for_medal = score.ranked or (score.leaderboard_eligible and policy.status == BeatmapRankStatus.LOVED)
    if not eligible_for_medal or (not policy.pp_enabled and policy.status != BeatmapRankStatus.LOVED):
        return False
