...
from app.service.beatmap_ranking_command_service import execute_ranking_command
...
class Bot:
    ...
    async def send_message(self, channel: ChatChannel, content: str, session: AsyncSession | None = None) -> None:
        ...
        await redis_message_system._store_to_redis(msg.message_id, channel_id, resp)
...
@bot.command("rank")
async def _rank(user: User, args: list[str], session: AsyncSession, channel: ChatChannel) -> str:
    """Apply an owner-authorized local Ranked policy."""

    return await execute_ranking_command("rank", user, args, session)


@bot.command("love")
async def _love(user: User, args: list[str], session: AsyncSession, channel: ChatChannel) -> str:
    """Apply an owner-authorized local Loved policy."""

    return await execute_ranking_command("love", user, args, session)


@bot.command("unrank")
async def _unrank(user: User, args: list[str], session: AsyncSession, channel: ChatChannel) -> str:
    """Clear an owner-authorized local ranking policy."""

    return await execute_ranking_command("unrank", user, args, session)
