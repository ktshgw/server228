...
from app.features.somsai.services.somsai_collector_service import collector_preview, collector_source, fetch_collector_source
from app.features.somsai.services.somsai_pool_service import delete_pool, get_playable_pool_slots, pool_payload, preview_pool, save_pool
...
class SomsaiPoolTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_http_import_refetches_source_forces_draft_and_audits(self):
        ...
        from app.features.somsai.routers.somsai_admin import create_somsai_import
        ...
        with (
            ...
            patch("app.features.somsai.routers.somsai_admin.preview_collector", AsyncMock(return_value=imported)) as source,
            patch("app.features.somsai.routers.somsai_admin.get_fetcher", AsyncMock(return_value=None)),
        ):
    ...
    async def test_failed_import_rolls_back_pool_and_audit(self):
        ...
        with (
            patch("app.router.private.admin_panel._expected_origin", return_value="http://test"),
            patch(
                "app.features.somsai.routers.somsai_admin.preview_collector",
...
class CollectorTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_redirects_rejected_and_body_size_bounded(self):
        for response in (
            ...
            with patch(
                "app.features.somsai.services.somsai_collector_service.httpx.AsyncClient",
    ...
    async def test_route_authorization_precedes_upstream_requests(self):
        from app.features.somsai.routers.somsai_admin import preview_somsai_import

        request = SomsaiImportRequest(url="https://osucollector.com/tournaments/123")
        with (
            patch("app.features.somsai.routers.somsai_admin._require_csrf", side_effect=HTTPException(403)),
            patch("app.features.somsai.routers.somsai_admin.preview_collector", new_callable=AsyncMock) as upstream,
        ):
            with self.assertRaises(HTTPException):
