...
class RankingApiValidationTests(unittest.TestCase):
    ...
    def test_only_visible_local_rank_is_restored_by_derank(self) -> None:
        ...
        local_rank = EffectiveBeatmapPolicy(
            status=BeatmapRankStatus.LOVED,
            leaderboard_enabled=True,
            pp_enabled=False,
            source=EffectivePolicySource.BEATMAPSET,
        )
        local_graveyard = EffectiveBeatmapPolicy(
        ...
        assert _is_effective_local_rank(local_rank)
        assert not _is_effective_local_rank(local_graveyard)
