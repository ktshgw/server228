...
Этот каталог содержит маленький проверяемый launcher для Windows x64. Он запускает обычный официальный `osu!.exe`, но передаёт endpoints приватного soms-сервера через EnhancedAuth.
...
`ClientId` и client secret принадлежат **вашему soms OAuth client**, а не официальному аккаунту osu!. Пароль пользователя сюда никогда не вводится. Secret передаётся через переменную процесса:
...
Для закрытого круга друзей этот путь подходит как PoC. Для долгоживущего сервера лучше перейти на минимальный собственный fork soms client, чтобы endpoints и отдельное имя хранилища были частью проверяемого исходного кода, а не runtime-патчем.
