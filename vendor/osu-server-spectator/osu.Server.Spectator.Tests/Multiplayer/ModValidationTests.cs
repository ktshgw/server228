...
namespace osu.Server.Spectator.Tests.Multiplayer
{
    public class ModValidationTests : MultiplayerTest
    {
        ...
        [Fact]
        public async Task SOMSDoubleTimeCanBeRequiredOrAllowed()
        {
            await Hub.JoinRoom(ROOM_ID);
        ...
        public async Task SOMSGuestCanChoosePersonalSpeed(bool freestyle, bool nightcore)
        {
            await Hub.JoinRoom(ROOM_ID);
            await Hub.EditPlaylistItem(new MultiplayerPlaylistItem
            {
                ...
                RulesetID = 0,
                Freestyle = freestyle,
                AllowedMods = freestyle ? [] : [new APIMod(new OsuModHidden())],
            });
            SetUserContext(ContextUser2);
            await Hub.JoinRoom(ROOM_ID);
            ModRateAdjust speed = nightcore ? new OsuModNightcore() : new OsuModDoubleTime();
            speed.SpeedChange.Value = 1.3;
            var chosen = new APIMod(speed);
            await Hub.ChangeUserMods([chosen]);
            using var usage = await Hub.GetRoom(ROOM_ID);
            Assert.Equal(chosen, usage.Item!.Users.Single(u => u.UserID == USER_ID_2).Mods.Single());
            Assert.Empty(usage.Item.Users.Single(u => u.UserID == USER_ID).Mods);
        }

        [Fact]
        public async Task SOMSPersonalSpeedCannotStackWithRequiredSpeed()
        {
            await Hub.JoinRoom(ROOM_ID);
            await Hub.EditPlaylistItem(new MultiplayerPlaylistItem
            {
            ...
            await Assert.ThrowsAsync<InvalidStateException>(() => Hub.ChangeUserMods([new APIMod(new OsuModNightcore())]));
        }
    }
}
