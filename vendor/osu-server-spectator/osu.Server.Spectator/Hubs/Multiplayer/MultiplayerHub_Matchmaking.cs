    ...
    public partial class MultiplayerHub : IMatchmakingServer
    {
        ...
        public async Task<MatchmakingPool[]> GetMatchmakingPoolsOfType(MatchmakingPoolType type)
        {
            using (var db = databaseFactory.GetInstance())
            {
                return (await db.GetActiveMatchmakingPoolsAsync())
                       .Where(p => p.type != matchmaking_pool_type.ranked_play || p.lobby_size == 2)
        ...
        public async Task MatchmakingJoinQueue(int poolId)
        {
            using (var db = databaseFactory.GetInstance())
            {
                ...
                if ((await db.GetMatchmakingPoolAsync((uint)poolId))?.type == matchmaking_pool_type.ranked_play)
                {
        ...
        public Task<MatchmakingIssueDuelResponse> MatchmakingIssueDuel(MatchmakingIssueDuelRequest request)
            => throw new InvalidStateException("Иди в обычный лазер");

        public Task<MatchmakingAcceptDuelResponse> MatchmakingAcceptDuel(MatchmakingAcceptDuelRequest request)
            => throw new InvalidStateException("Иди в обычный лазер");
