...
namespace osu.Server.Spectator.Hubs.Multiplayer
{
    public class MultiplayerRoomController : IMultiplayerRoomController
    {
        ...
        private readonly SomsaiNativeRoomLeaseService? nativeRoomLeases;
        ...
        public MultiplayerRoomController(
            ...
            SomsaiNativeRoomLeaseService? nativeRoomLeases = null)
        {
            this.rooms = rooms;
            ...
            this.nativeRoomLeases = nativeRoomLeases;
        ...
        {
            if (rooms.GetEntityUnsafe(roomId) == null)
            {
                // SOMSAI rooms are reserved in the app before a native host exists.
                // Verify the authoritative roster before creating native tracking.
                ...
                using (var roomUsage = await rooms.GetForUse(roomId, isNewRoom))
                {
                    ...
                    try
                    {
                        ...
                        if (nativeRoomLeases != null && roomUser.Role == MultiplayerRoomUserRole.Player
                            && room.Settings.MatchType != MatchType.RankedPlay && room.MatchController is not Standard.SomsaiMatchController)
                            await nativeRoomLeases.Claim(roomId, roomUser.UserID, () => leaveUnreservedRoom(roomId, roomUser.UserID));
                    ...
                    catch
                    {
                        ...
                        finally
                        {
                            ...
                            if (nativeRoomLeases != null)
                                await nativeRoomLeases.Release(roomId, userState.UserId);
                        }
        ...
        private async Task removeUserFromRoom(IMultiplayerUserState state, ItemUsage<ServerMultiplayerRoom> roomUsage, int removingUserId, bool forceCloseOnEmpty = false)
        {
            ...
            finally
            {
                if (roomId != null)
                {
                    ...
                    if (nativeRoomLeases != null)
                        await nativeRoomLeases.Release(roomId.Value, state.UserId);
                }
            }
        }
        ...
        private async Task leaveUnreservedRoom(long roomId, int userId)
        {
            using var user = await players.TryGetForUse(userId);
            if (user?.Item?.CurrentRoomID != roomId)
                return;
            using var room = await rooms.TryGetForUse(roomId);
            if (room?.Item == null)
                return;
            await LeaveRoom(user.Item, room);
        }
