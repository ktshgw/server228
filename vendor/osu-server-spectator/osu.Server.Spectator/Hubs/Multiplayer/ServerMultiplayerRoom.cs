...
namespace osu.Server.Spectator.Hubs.Multiplayer
{
    public class ServerMultiplayerRoom : MultiplayerRoom
    {
                                                                                       ...
                                                                                       RulesetManager rulesetManager, IMatchmakingQueueBackgroundService matchmakingService)
        {
            ...
            if (room.Settings.MatchType != pool.type.ToMatchType())
                throw new InvalidOperationException("Failed to initialise the matchmaking room (pool and room types differ).");
        ...
        {
            public Task<matchmaking_pool[]> GetMatchmakingPoolsAsync() => throw new NotSupportedException();
            public Task<database_beatmap[]> GetMatchmakingBeatmapsAsync(int rulesetId, int variant, uint poolId) => throw new NotSupportedException();
            public Task<bool> IsMatchmakingPoolAvailableAsync(uint poolId) => throw new NotSupportedException();
            public Task<RankedDodgeStatus> GetRankedDodgeStatusAsync(int userId) => throw new NotSupportedException();
            public Task<RankedDodgeStatus> RegisterRankedDodgeAsync(long roomId, int userId) => throw new NotSupportedException();
            public Task<RankedPartyReservation> ReserveRankedPartyAsync(int userId, int poolId) => throw new NotSupportedException();
            public Task RenewPartyReservationAsync(string reservationId) => throw new NotSupportedException();
            public Task ReleasePartyReservationAsync(string reservationId) => throw new NotSupportedException();
        ...
        private async Task changeUserStyle(MultiplayerRoomUser user, int? beatmapId, int? rulesetId)
        {
            ...
            if (!MatchController.CurrentItem.ValidateUserMods(user, user.Mods, out var validMods, RulesetManager, MatchController is not SomsaiMatchController))
            {
        ...
        private async Task changeUserMods(MultiplayerRoomUser user, IEnumerable<APIMod> newMods)
        {
            ...
            if (!MatchController.CurrentItem.ValidateUserMods(user, newModList, out var validMods, RulesetManager, MatchController is not SomsaiMatchController))
