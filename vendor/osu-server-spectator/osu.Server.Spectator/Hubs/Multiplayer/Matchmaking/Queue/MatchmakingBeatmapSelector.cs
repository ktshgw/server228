        ...
        public static async Task<MatchmakingBeatmapSelector> Initialise(matchmaking_pool pool, IDatabaseFactory dbFactory)
        {
            using (var db = dbFactory.GetInstance())
            {
                ...
                Dictionary<int, matchmaking_pool_beatmap> globalBeatmaps =
                    (await db.GetMatchmakingGlobalPoolBeatmapsAsync(pool.ruleset_id, pool.variant_id, pool.id))
                ...
                if (pool.use_dmr)
                    poolBeatmaps = (await db.GetMatchmakingPoolBeatmapsAsync(pool.id))
                        .Where(b => globalBeatmaps.ContainsKey(b.beatmap_id))
                        .Select(b =>
                        {
                            if (string.IsNullOrWhiteSpace(b.mods) || b.mods.Trim() == "[]")
                                b.mods = string.Empty;
                            return b;
                        })
