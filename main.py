...
from app.features.somsai.routers.somsai_lio import router as somsai_lio_router
...
async def lifespan(app: FastAPI):
    ...
    # === on startup ===
    startup_logger.info("Starting soms-server")
    ...
    startup_logger.info("soms-server startup completed")

    yield

    # === on shutdown ===
    shutdown_logger.info("Stopping soms-server")
    ...
    for client in redis_clients.values():
        await client.close()
    shutdown_logger.info("soms-server shutdown completed")


desc = f"""SOMS! is a private osu!(lazer) server with its own website, client integration, multiplayer services, and SOMSAI tournament mode.

## Endpoint Specifications

All v2 APIs begin with `/api/v2/`, while all v1 APIs start with `/api/v1/` (direct access to `/api` for v1 APIs will redirect).
All additional APIs provided by soms-server (SOMS API) begin with `/api/private/`. All additional APIs provided by plugins begin with `/api/plugins/<plugin-id>/`
...
async def private_client_manifest():
    ...
    content = {
        ...
        "credential_target": f"SomsPrivateOsu/{settings.server_url.host}",
        "client": {
