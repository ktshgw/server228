...
from soms_migrations.model import ContextObject, SomsServerConfig
from soms_migrations.utils import detect_soms_server_path, get_plugin_id
...
def _ensure_env(obj: ContextObject, autogenerate: bool = False):
                ...
                if obj["plugin_id"] and plugin_path and autogenerate:
                    ...
                    txt = txt.replace(
                        ...
                        (
                            ...
                            f"sys.path.insert(0, r'{obj['soms_server_path'].as_posix()}'); "
    ...
    alembic_config.set_section_option(
        alembic_config.config_ini_section, "sqlalchemy.url", obj["soms_server_config"].database_url
    )
    try:
        yield
    finally:
...
@click.option(
    ...
    default=None,
    help=(
        "The directory path to soms-server. If not provided, "
...
def soms_migrate(
    ...
    config: Path | None,
    name: str,
    soms_server_path: Path | None,
    plugin_path: Path | None,
):
    config_path: Path | None = config
    if soms_server_path is None:
        soms_server_path = detect_soms_server_path()
        if soms_server_path is None:
            raise click.ClickException("Could not detect soms-server path. Please provide it via --path option.")
        click.echo(f"Detected soms-server at {soms_server_path}")
    if config_path is None:
        config_path = soms_server_path / "alembic.ini"
    ...
    soms_config = SomsServerConfig(_env_file=soms_server_path / ".env")  # pyright: ignore[reportCallIssue]

    obj: ContextObject = {
        "soms_server_path": soms_server_path,
        "alembic_config": alembic_config,
        "plugin_path": cwd,
        "plugin_id": plugin_id,
        "soms_server_config": soms_config,
    }
    ctx.obj = obj


@soms_migrate.command()
@click.option(
    "-r",
    "--rev-range",
    default=None,
...
def upgrade_all(ctx: click.Context):
    """Upgrade the soms-server and all plugins' databases to the latest version."""
    obj: ContextObject = ctx.obj

    if obj["soms_server_path"] != Path.cwd().resolve():
        raise click.ClickException("Please run this command from the soms-server root directory.")

    # Upgrade soms-server
    click.echo("Upgrading soms-server...")
    with _ensure_env(obj):
        alembic.command.upgrade(obj["alembic_config"], "head")

    # Upgrade plugins
    for plugin_dir in obj["soms_server_config"].plugin_dirs:
        plugins_path = obj["soms_server_path"].joinpath(plugin_dir)
