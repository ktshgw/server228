...
def detect_soms_server_path() -> Path | None:
    """Detect the soms server path from the current working directory to parents.

    Returns:
        The path to the soms server, or None if not found.
    """
    cwd = Path.cwd()
    for path in [cwd, *list(cwd.parents)]:
        if (pyproject := (path / "pyproject.toml")).exists():
            try:
                ...
                continue
            if "project" in content and content["project"].get("name") == "soms-server":
                return path.resolve()
