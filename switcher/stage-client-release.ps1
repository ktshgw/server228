...
$destination = Join-Path $workspace "server\static\client\$OsuVersion"
$sharedLauncherConfig = Join-Path $workspace "launcher\shared-launcher.json"
...
$updatedConfigs = @()
foreach ($launcherConfigPath in @($sharedLauncherConfig, (Join-Path $workspace "launcher\launcher.json"))) {
