...
foreach (string ruleset in new[] { "Osu", "Taiko", "Catch", "Mania" })
{
    ...
    foreach (string modName in new[] { "DoubleTime", "Nightcore", "DifficultyAdjust", "Classic", "Autoplay", "HalfTime" })
    {
        ...
        foreach (var (url, disableExtensions, expected) in new[] {
        ...
        })
        {
            ...
            configType.GetProperty("DisableServerExtensions")!.SetValue(config, disableExtensions);
            bool actual = (bool)ranked.GetValue(mod)!;
            if (actual != expected)
                throw new Exception($"{ruleset}/{modName} {url} disableExtensions={disableExtensions}: ranked={actual}, expected={expected}");
            checks++;
        }
    }
}
...
})
{
    ...
    if (count != expected)
        throw new Exception($"{url} disableExtensions={disableExtensions}: registered skin slots={count}, expected={expected}");
}
...
try
{
    ...
    configType.GetProperty("DisableServerExtensions")!.SetValue(config, false);
