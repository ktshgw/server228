...
$ServerDirectory = $WorkspaceDirectory
...
$LauncherTemplatePath = Join-Path $WorkspaceDirectory "client\launcher\launcher.example.json"
$LauncherConfigurationPath = Join-Path $WorkspaceDirectory "client\launcher\launcher.json"
$SharedLauncherConfigurationPath = Join-Path $WorkspaceDirectory "client\launcher\shared-launcher.json"
...
function Sync-LauncherConfiguration {
    ...
    if (-not (Test-Path -LiteralPath $LauncherTemplatePath -PathType Leaf)) {
        throw "client\launcher\launcher.example.json is missing. Restore the workspace before starting."
    }
    ...
    try {
        ...
        throw "client\launcher\launcher.example.json is invalid. Restore the workspace before starting."
    }
    if (-not (Test-LauncherConfigurationShape $templateSettings)) {
        throw "client\launcher\launcher.example.json has an unsupported structure. Restore the workspace before starting."
    }
    ...
    if ($launcherExists) {
        try {
            ...
            if (-not $AllowOriginChange) {
                throw "client\launcher\launcher.json is invalid. Run START_SERVER.bat Setup to back it up and regenerate it."
            }
    ...
    if (-not (Test-LauncherConfigurationShape $settings)) {
        if (-not $AllowOriginChange) {
            throw "client\launcher\launcher.json has an unsupported structure. Run START_SERVER.bat Setup to back it up and regenerate it."
        }
        if ($launcherExists) {
            $backupName = "launcher.json.invalid-$((Get-Date).ToUniversalTime().ToString('yyyyMMdd-HHmmss')).bak"
            $backupPath = Join-Path (Split-Path -Parent $LauncherConfigurationPath) $backupName
    ...
    if (-not [string]::IsNullOrWhiteSpace($existingOrigin) -and
        ...
        if (-not $AllowOriginChange) {
            throw "client\launcher\launcher.json targets another server. Run START_SERVER.bat Setup to change it deliberately."
        }
    ...
    if (Test-Path -LiteralPath $SharedLauncherConfigurationPath -PathType Leaf) {
        try {
            ...
            throw "client\launcher\shared-launcher.json is invalid. Restore it before starting the server."
        }

        if ($null -eq $sharedSettings.Server -or $null -eq $sharedSettings.Injection) {
            throw "client\launcher\shared-launcher.json has an unsupported structure."
        }
...
function Initialize-Environment {
    ...
    if (-not (Test-Path -LiteralPath $EnvironmentTemplatePath -PathType Leaf)) {
        throw "Missing .env.example. The workspace is incomplete."
    }
    ...
    foreach ($specification in $secretSpecifications) {
        ...
        if ([bool]$specification.Database -and $databaseVolumeExists) {
            throw "The existing MySQL volume needs its original strong $name. Restore the old .env; never guess or regenerate a database password."
        }
    ...
    if ($isNewEnvironment) {
        Write-Step "Created .env atomically from the safe template."
    }
    Sync-LauncherConfiguration $serverDomain -AllowOriginChange:$Reconfigure
    Write-Step "First-run configuration is ready. Secrets were saved only in .env and were not printed."
}

function Assert-EnvironmentReady {
    if (-not (Test-Path -LiteralPath $EnvironmentPath -PathType Leaf)) {
        throw ".env does not exist. Run START_SERVER.bat once to create it."
    }
    ...
    foreach ($name in $required) {
        if (-not $values.ContainsKey($name) -or (Test-Placeholder ([string]$values[$name]))) {
            throw ".env is incomplete at $name. Run START_SERVER.bat Setup."
        }
    }
    ...
    foreach ($name in $localSecretPatterns.Keys) {
        ...
        if (-not $uniqueSecrets.Add($value)) {
            throw "MYSQL/JWT/private-client/interoperability secrets must all be different. Fix .env before starting."
        }
    }
...
function Assert-RequiredServicesRunning {
    ...
    $required = @("app", "mysql", "redis", "spectator", "nginx", "caddy")
...
function Assert-ComponentReadiness {
    Write-Step "Checking the spectator gateway..."
    $deadline = [DateTime]::UtcNow.AddSeconds(45)
    while ([DateTime]::UtcNow -lt $deadline) {
        if (Test-SpectatorService) {
            return
        }
        Start-Sleep -Seconds 3
    }
    throw "The spectator gateway is not ready. Run START_SERVER.bat Logs and inspect spectator."
}
