...
OUTPUT = WORKSPACE / "client/enhanced-auth/osu.Game.Rulesets.EnhancedAuth/Resources/StableFonts"
SIZE = 100
...
class Face:
    def __init__(self, path: Path):
        ...
        self.units = int(getattr(self.ttf["head"], "unitsPerEm"))
...
def build(name: str, source_path: Path, fallback_path: Path, bold: bool) -> dict:
    ...
    for code in CODEPOINTS:
        ...
        width, height = max(1, round(right - left)), max(1, round(bottom - top))
    ...
    required = {ord(c) for c in "ИгратьРедакторНастройкиВыходМодыКоллекцииЁёЙй0123456789!?%"}
    ...
    kern_block = b"".join(
        struct.pack("<IIh", first, second, amount) for (first, second), amount in sorted(kerning.items())
    )
...
def main() -> None:
    ...
    fonts = [
        build("SomsAller-Regular", args.stable_assets / "Aller.ttf", args.fallback, False),
    ...
    for font in fonts:
        total_bytes = sum(file["bytes"] for file in font["files"])
        print(
            f"{font['name']}: {font['glyph_count']} glyphs, {len(font['fallback_glyphs'])} explicit fallback glyphs, "
            f"{font['pages']} pages, {font['kerning_pairs']} kerning pairs, {total_bytes} bytes"
        )
