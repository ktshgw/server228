...
$environmentPath = Join-Path $workspaceDirectory ".env"
$launcherPath = Join-Path $workspaceDirectory "client\launcher\Start-SharedPrivateOsu.ps1"
$serverScriptPath = Join-Path $workspaceDirectory "scripts\server\easy-server.ps1"
...
function Read-DotEnvValue {
    ...
    throw "Missing $Name in .env. Run START_SERVER.bat first."
}
...
if ($clientSecret.Length -lt 16) {
    throw "OSU_CLIENT_SECRET in .env is invalid."
}
