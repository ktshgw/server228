...
namespace osu.Server.Spectator.Hubs.Multiplayer.Matchmaking.Queue
{
    public interface IMatchmakingQueueBackgroundService : IHostedService
    {
        ...
        Task AddReservedToQueueAsync(MultiplayerClientState[] states, int poolId, RankedPartyReservation reservation);
        void TrackPartyReservation(RankedPartyReservation reservation, int poolId);
        void AbandonUnusedPartyReservation(string reservationId);
        ...
        /// <summary>Persist an early Ranked cancellation and penalise its initiating player.</summary>
        Task RegisterRankedDodgeAsync(long roomId, int userId);
    }
}
