...
internal sealed partial class LoadGame : OsuGameBase
{
    ...
    private int stage, busyPhase, createRequests;
    private ApplySomsAiActionRequest? pendingCreate;
    private int frames;
    private int avatarWait;
    private Drawable[] teamAvatars = Array.Empty<Drawable>();
    ...
    public LoadGame(bool visual, string output, string profile)
    {
        ...
        {
            if (request is not ApplySomsAiActionRequest create) return false;
            createRequests++; pendingCreate = create; return true;
        };
    }
    ...
    protected override void Update()
    {
        ...
        switch (stage++)
        {
            ...
            case 2:
                ...
                break;
            case 3:
                var createButton = (osu.Game.Graphics.UserInterfaceV2.RoundedButton)typeof(SomsAiScreen).GetField("createRoomButton", BindingFlags.Instance | BindingFlags.NonPublic)!.GetValue(dashboard)!;
                if (busyPhase == 0)
                {
                    var creationForm = (Drawable)typeof(SomsAiScreen).GetField("customForm", BindingFlags.Instance | BindingFlags.NonPublic)!.GetValue(dashboard)!;
                    typeof(SomsAiScreen).GetMethod("revealForm", BindingFlags.Instance | BindingFlags.NonPublic)!.Invoke(dashboard, new object[] { creationForm });
                    ((FormTextBox)typeof(SomsAiScreen).GetField("customMmr", BindingFlags.Instance | BindingFlags.NonPublic)!.GetValue(dashboard)!).Current.Value = "1500";
                    ((FormCheckBox)typeof(SomsAiScreen).GetField("customWithBots", BindingFlags.Instance | BindingFlags.NonPublic)!.GetValue(dashboard)!).Current.Value = true;
                    createButton.TriggerClick(); createButton.TriggerClick();
                    busyPhase = 1; stage--; return;
                }
                if (busyPhase == 1)
                {
                    ((osu.Game.Graphics.Containers.OsuScrollContainer)typeof(SomsAiScreen).GetField("oceanScroll", BindingFlags.Instance | BindingFlags.NonPublic)!.GetValue(dashboard)!).ScrollToEnd(false);
                    if (pendingCreate == null || createRequests != 1 || createButton.Enabled.Value || createButton.Text.ToString() != "Создаём комнату…") throw new Exception("Create request must be single and button disabled");
                    var spinner = (Drawable)typeof(SomsAiScreen).GetField("createRoomSpinner", BindingFlags.Instance | BindingFlags.NonPublic)!.GetValue(dashboard)!;
                    if (!spinner.IsLoaded || !spinner.IsPresent) { stage--; return; }
                    snapshot("03a-creating-room", () => pendingCreate!.Fail(new Exception("Fixture server failure")));
                    busyPhase = 2; stage--; return;
                }
                if (!createButton.Enabled.Value) { stage--; return; }
                Console.WriteLine("PASS creation spinner, double-click guard and recovery after failure");
                checkDashboardBounds();
            ...
            case 5:
                ...
                checkMapBoard();
                teamAvatars = currentTeamAvatars();
            ...
            case 7:
                ...
                var bannedCard = cardList.First(c => (string)c.GetType().GetProperty("SlotStatus")!.GetValue(c)! == "banned");
                var banShade = (Drawable)bannedCard.GetType().GetField("banShade", BindingFlags.Instance | BindingFlags.NonPublic)!.GetValue(bannedCard)!;
                if (banShade.Alpha < .7f) { stage--; return; }
                var ordered = ((IEnumerable<Drawable>)typeof(CompositeDrawable).GetProperty("InternalChildren", BindingFlags.Instance | BindingFlags.NonPublic)!.GetValue(banShade.Parent)!).ToArray();
                var statsText = (Drawable)bannedCard.GetType().GetField("statsLine", BindingFlags.Instance | BindingFlags.NonPublic)!.GetValue(bannedCard)!;
                if (Array.IndexOf(ordered, statsText.Parent) >= Array.IndexOf(ordered, banShade)) throw new Exception("Ban shade must dim text as well as cover");
                for (int poll = 0; poll < 10; poll++)
                {
                    var repeated = (SomsAiState)typeof(SomsAiScreen).GetField("state", BindingFlags.Instance | BindingFlags.NonPublic)!.GetValue(dashboard)!;
                    repeated.Match!.Wins[0] = poll % 2;
                    repeated.Match.Teams[0].Members[0].Ready = poll % 2 == 1;
                    setDashboardState(Newtonsoft.Json.JsonConvert.SerializeObject(repeated));
                    if (!currentTeamAvatars().SequenceEqual(teamAvatars)) throw new Exception("Polling recreated captain avatars");
                }
                Console.WriteLine("PASS banned card dims cover and text; captain avatars survive 10 score/readiness refreshes");
    ...
    private Drawable[] currentTeamAvatars() => ((Array)typeof(SomsAiScreen).GetField("teamHeaders", BindingFlags.Instance | BindingFlags.NonPublic)!.GetValue(dashboard)!)
        .Cast<object>().Select(header => ((Container)header.GetType().GetField("avatar", BindingFlags.Instance | BindingFlags.NonPublic)!.GetValue(header)!).Children.Single()).ToArray();
