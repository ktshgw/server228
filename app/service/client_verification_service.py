...
BUNDLED_HASHES = Path(__file__).resolve().parents[2] / "static" / "client" / "client_versions.json"
...
class ClientVerificationService:
    ...
    async def load_from_disk(self, first_load: bool = False) -> None:
        ...
        async with self._lock:
            self.versions.clear()
            try:
                source = HASHES_DIR if HASHES_DIR.is_file() else BUNDLED_HASHES
                if not source.is_file():
                    if not first_load:
                        logger.warning("Client version list file does not exist")
                    return
