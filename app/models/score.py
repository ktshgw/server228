...
class GameMode(StrEnum):
    ...
    def to_special_mode(self, mods: list[APIMod] | list[str]) -> "GameMode":
        if settings.soms_osu_assist_modes:
            if self != GameMode.OSU:
                return self
            acronyms = {m["acronym"] if isinstance(m, dict) else m for m in mods}
            if "AP" in acronyms:
                return GameMode.OSUAP
            if "RX" in acronyms:
                return GameMode.OSURX
            return self
        ...
        if self == GameMode.OSU and "AP" in mods and settings.enable_ap:
            return GameMode.OSUAP
    ...
    def check_ruleset_version(self, hash: str) -> RulesetCheckResult:
        ...
        return RulesetCheckResult(
            ...
            download_url=DOWNLOAD_URL.format(version=latest_version) if DOWNLOAD_URL and not is_current else None,
        )
