...
class Settings(BaseSettings):
    ...
    # jwt
    shared_interop_secret: Annotated[str, Field(default=""), "spectator"]
    ...
    default_country_code: Annotated[str, Field(default="XX", pattern=r"^[A-Z]{2}$"), "server"]
    ...
    totp_service_name: Annotated[str, Field(default="SOMS! Lazer Server"), "verification"]
    ...
    # game
    soms_osu_assist_modes: bool = True
    ...
    # Keep private users outside the range used by official osu! accounts when
    # both servers intentionally share one lazer client.realm.
    private_user_id_base: Annotated[int, Field(default=3, ge=3, le=2_000_000_000), "game"]
    ...
    # calculator
    calculator: Annotated[str, Field(default="rosu"), "calculator"]
    calculator_config: Annotated[dict[str, Any], Field(default={}), "calculator"]
    ...
    custom_asset_domain: Annotated[str, Field(default="assets.ppy.sh"), "asset_proxy"]
    ...
    enable_ranked_catalogue_sync: Annotated[bool, Field(default=True), "beatmap_sync"]
    ...
    def validate_global_pp_configuration(self) -> "Settings":
        """PP requires the leaderboard index used for best-score deltas."""

        if self.enable_all_beatmap_pp and not self.enable_all_beatmap_leaderboard:
            raise ValueError("ENABLE_ALL_BEATMAP_PP=true requires ENABLE_ALL_BEATMAP_LEADERBOARD=true")
        return self
