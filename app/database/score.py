...
from .playlist_best_score import (
    PlaylistBestScore,
    get_position as get_playlist_score_position,
    partition_scores_around,
)
...
from .statistics import change_grade_count
...
from sqlalchemy import Boolean, CheckConstraint, Column, DateTime, Index, Integer, TextClause, exists
...
if TYPE_CHECKING:
    ...
    from app.service.beatmap_ranking_service import EffectiveBeatmapPolicy
...
class ScoreModel(AsyncAttrs, DatabaseModel[ScoreDict]):
    ...
    async def position(
        ...
        if playlist_id is not None and room_id is not None:
            return await get_playlist_score_position(room_id, playlist_id, score.id, session) or None
        ...
        scores = (
            await session.exec(
                select(PlaylistBestScore).where(
                    ...
                    col(PlaylistBestScore.score).has(col(Score.passed).is_(True)) if is_playlist else True,
                )
            )
        ).all()
        higher, lower, more_higher, more_lower = partition_scores_around(scores, _score.id)
        if not any(score.score_id == _score.id for score in scores):
            return None

        async def transform_page(
            page: list[PlaylistBestScore],
            *,
            sort: str,
            has_more: bool,
        ) -> MultiplayerScores:
            responses = [
                await ScoreModel.transform(score.score, includes=ScoreModel.MULTIPLAYER_BASE_INCLUDES) for score in page
            ]
            result = MultiplayerScores(scores=responses, params={"limit": 10, "sort": sort})
            if has_more and page:
                result.cursor = {
                    "total_score": page[-1].total_score,
                    "score_id": page[-1].score_id,
                }
            return result

        return ScoreAround(
            higher=await transform_page(higher, sort="score_asc", has_more=more_higher),
            lower=await transform_page(lower, sort="score_desc", has_more=more_lower),
        )
...
class Score(ScoreModel, table=True):
    ...
    __table_args__ = (
        ...
        CheckConstraint("pinned_order >= 0", name="ck_scores_pinned_order_nonnegative"),
    )
    ...
    # Immutable eligibility snapshots taken when the play is submitted. The
    # background worker also checks the current policy; requiring both sides
    # prevents a later local rank from promoting an older play.
    leaderboard_eligible: bool = Field(
        default=False,
        exclude=True,
        sa_column=Column(Boolean, nullable=False, server_default=text("0")),
    )
    ranked_score_eligible: bool = Field(
        default=False,
        exclude=True,
        sa_column=Column(Boolean, nullable=False, server_default=text("0")),
    )
    # Distinguishes rows created with immutable eligibility snapshots from
    # legacy rows whose false defaults do not prove they were ineligible.
    ranking_eligibility_snapshotted: bool = Field(
        default=True,
        exclude=True,
...
async def calculate_user_pp(
    ...
    *,
    ...
    pp_sum = 0
    acc_sum = 0
    bps = await get_user_best_pp(session, user_id, mode, for_update=for_update)
    from app.service.negative_pp_service import negative_map_ids

    negative_maps = await negative_map_ids(session, [score.beatmap_id for score in bps])
    for i, s in enumerate(bps):
        pp_sum += calculate_weighted_pp(s.pp, i) * (-1 if s.beatmap_id in negative_maps else 1)
...
async def get_user_best_pp(
    ...
    *,
    ...
    statement = (
        ...
        .order_by(col(BestScore.pp).desc(), col(BestScore.score_id).desc())
        .limit(limit)
    )
    if for_update:
...
async def process_score(
    ...
    """  # noqa: E501
    # Keep this import local: importing app.service while app.database is still
    # initialising pulls in room/fetcher services and creates a package cycle.
    ...
    # Snapshot eligibility under the same per-set lock used by policy changes,
    # synchronisation, and the background score worker.  The request has
    # already performed ordinary reads, so use current locking reads here to
    # avoid inheriting a stale MySQL REPEATABLE READ view.
    locked_beatmapset = (
        await session.exec(
            select(Beatmapset)
            .where(Beatmapset.id == beatmap.beatmapset_id)
            .with_for_update()
            .execution_options(populate_existing=True)
        )
    ).first()
    if locked_beatmapset is None:
        raise ValueError(f"Beatmapset {beatmap.beatmapset_id} does not exist")
    locked_beatmap = (
        await session.exec(
            select(Beatmap).where(Beatmap.id == beatmap_id).with_for_update().execution_options(populate_existing=True)
        )
    ).first()
    if locked_beatmap is None:
        raise ValueError(f"Beatmap {beatmap_id} does not exist")
    beatmap = locked_beatmap
    beatmap_policy = await get_effective_beatmap_policy(session, beatmap, for_update=True)
    submitted_checksum = score_token.beatmap_checksum
    checksum_matches = submitted_checksum == beatmap.checksum
    leaderboard_eligible = checksum_matches and (
        beatmap_policy.leaderboard_enabled or settings.enable_all_beatmap_leaderboard
    )
    ranked = checksum_matches and (beatmap_policy.pp_enabled or settings.enable_all_beatmap_pp)
    ranked_score_eligible = checksum_matches and (
        (beatmap_policy.status.ranked() and leaderboard_eligible) or settings.enable_all_beatmap_pp
    )
    ...
    score = Score(
        ...
        ranked_score_eligible=ranked_score_eligible,
        ranking_eligibility_snapshotted=True,
    ...
    # The caller links this row to the one-time ScoreToken and commits both
    # atomically. Committing here would release the token lock and allow a
    # duplicate concurrent submission to create a second score.
    await session.flush()
    await session.refresh(score)
    return score


async def _process_score_pp(
    score: "Score",
    session: AsyncSession,
    redis: Redis,
    fetcher: "Fetcher",
    *,
    ...
    # A cached calculation is not permission to award PP. The mod policy may
    # have changed since the score was imported or first processed.
    can_get_pp = score.passed and score.ranked and mods_can_get_pp(int(score.gamemode), score.mods)
    if not can_get_pp:
        score.pp = 0
        logger.debug("Skipping PP for ineligible score {score_id}", score_id=score.id)
        return True
    if score.pp != 0:
        logger.debug(
            "Reusing PP calculation for score {score_id} | already set {pp:.2f}",
            score_id=score.id,
            pp=score.pp,
        )
    else:
        pp, successed = await pre_fetch_and_calculate_pp(
            ...
            redis,
            fetcher,
            raise_when_not_found=raise_when_not_found,
        )
        if not successed:
            if queue_on_failure:
...
async def _process_score_events(score: "Score", session: AsyncSession, previous_winner=None):
    ...
    if rank_global == 1 and previous_winner and previous_winner[1] != score.user_id:
        username = previous_winner[2]

        rank_lost_event = Event(
            created_at=utcnow(),
            type=EventType.RANK_LOST,
            user_id=previous_winner[1],
        )
        rank_lost_event.event_payload = {
            ...
            "user": {
                "username": username,
                "url": settings.web_url + "users/" + str(previous_winner[1]),
            },
        }
...
async def _process_statistics(
    ...
    score_token: int,
    beatmap_length: int,
    beatmap_policy: "EffectiveBeatmapPolicy",
    *,
    emit_playtime_event: bool = True,
):
    has_pp = score.ranked and (beatmap_policy.pp_enabled or settings.enable_all_beatmap_pp)
    has_leaderboard = score.leaderboard_eligible and (
        beatmap_policy.leaderboard_enabled or settings.enable_all_beatmap_leaderboard
    )
    ranked = score.ranked_score_eligible and (
        (beatmap_policy.status.ranked() and has_leaderboard) or settings.enable_all_beatmap_pp
    )
    ...
    add_to_db = False
    play_date = score.ended_at.date()
    mouthly_playcount = (
        await session.exec(
            select(MonthlyPlaycounts).where(
                MonthlyPlaycounts.user_id == user.id,
                MonthlyPlaycounts.year == play_date.year,
                MonthlyPlaycounts.month == play_date.month,
            )
        )
    ).first()
    if mouthly_playcount is None:
        mouthly_playcount = MonthlyPlaycounts(user_id=user.id, year=play_date.year, month=play_date.month)
        add_to_db = True
    statistics = None
    ...
    if difference > 0 and score.passed and ranked:
        change_grade_count(statistics, score.rank, 1)
        if previous_score_best is not None:
            change_grade_count(statistics, previous_score_best.rank, -1)
...
async def process_user(
    ...
    *,
    ...
    Args:
        ...
        beatmap_policy: Effective local/upstream policy controlling PP and leaderboard updates.
        emit_playtime_event: Notify a waiting live client through Redis. Historical imports disable this.
    """
    score_id = score.id
    user_id = user.id
    logger.info(
    ...
    pp_ready = True
    if score.ranked and (beatmap_policy.pp_enabled or settings.enable_all_beatmap_pp):
        pp_ready = await _process_score_pp(
            score,
            session,
            redis,
            fetcher,
            queue_on_failure=queue_pp_failure,
        )
    await session.flush()

    previous_winner = None
    if create_events:
        from app.features.somsai.services.soms_activity_service import current_winner

        # Serialize submissions on this difficulty until the best score and outbox commit together.
        await session.exec(select(Beatmap.id).where(Beatmap.id == score.beatmap_id).with_for_update())
        previous_winner = await current_winner(session, score.beatmap_id, score.gamemode)
    ...
    await _process_statistics(
        ...
        beatmap_length,
        beatmap_policy,
        emit_playtime_event=emit_playtime_event,
    )
    ...
    if score_ is None:
        ...
        return False
    if create_events:
        from app.features.somsai.services.soms_activity_service import stage_first_place

        await _process_score_events(score_, session, previous_winner)
        await stage_first_place(session, score_, previous_winner)
    if commit:
