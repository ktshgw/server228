...
from sqlmodel import JSON, Boolean, Column, DateTime, Field, Integer, Relationship, col, exists, func, select, text
...
class UserDict(TypedDict):
    ...
    global_rank: NotRequired[dict[str, int | None]]
...
class UserModel(DatabaseModel[UserDict]):
    ...
    avatar_url: str = ""
    country_code: str = Field(default="XX", max_length=2, index=True)
    ...
    # soms-extra
    soms_playmode: GameMode = GameMode.OSU
...
class User(AsyncAttrs, UserModel, table=True):
    ...
    # Keep the official-shaped game API ID in the collision-safe high range,
    # while exposing a small server-local number on the bundled websites.
    # This field deliberately lives only on the table model so it is never
    # added to osu!'s API response models by accident.
    ...
    # Server ownership is intentionally separate from the legacy ``priv``
    # bitfield: private-server ecosystems assign incompatible meanings to the
    # same bits, so a bit-based owner check can accidentally escalate staff.
    is_owner: bool = Field(
        default=False,
