...
@router.post("/rename", name="Rename user", tags=["User", "SOMS API"], description="Rename a user.")
    ...
    async def clear(current_user: User, fields: list[str]):
        ...
        for field in fields:
            if field in USER_PROFILE_FIELDS_WITH_WEBSITE:
                ...
                current_user.soms_playmode = GameMode.OSU
...
async def get_user_preference(
    ...
    return Preferences(
        ...
        playmode=current_user.soms_playmode,
...
async def change_user_preference(
    ...
    if request.playmode is not None:
        ...
        current_user.soms_playmode = request.playmode
