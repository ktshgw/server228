...
@router.get(
    ...
    responses={
        200: api_doc(
            ...
            [*User.CARD_INCLUDES, "statistics_rulesets"],
...
async def get_users(
    ...
    """
    # MultiplayerClient uses this endpoint specifically because, unlike the
    # lookup endpoint, it must contain ``statistics_rulesets``.  Do not use the
    # generic compact-user cache here: older/other endpoint entries omit those
    # statistics and make every participant rank disappear in lazer.
    requested_ids = list(dict.fromkeys(user_ids[:50]))
    if requested_ids:
        queried = (
            await session.exec(
                select(User).where(
                    col(User.id).in_(requested_ids),
                    col(User.id) != BANCHOBOT_ID,
                    ~User.is_restricted_query(col(User.id)),
                )
            )
        ).all()
        users_by_id = {user.id: user for user in queried}
        searched_users = [users_by_id[user_id] for user_id in requested_ids if user_id in users_by_id]
    else:
        searched_users = (
            await session.exec(
                select(User)
    ...
    includes = [*User.CARD_INCLUDES, "statistics_rulesets"]
    return {
        "users": [await UserModel.transform(user, includes=includes) for user in searched_users],
    }
...
async def lookup_users(
    ...
    request: Request,
    background_task: BackgroundTasks,
    user_ids: Annotated[list[int], Query(default_factory=list, alias="ids[]")],
    ruleset_id: Annotated[int | None, Query(description="Ruleset used for the compact global rank")] = None,
):
    """Return the compact user cards consumed by lazer's realtime user list.

    ``LookupUsersRequest`` deliberately has a different contract from the
    multiplayer ``/users/`` batch endpoint: it expects a top-level
    ``global_rank`` and does not need every ruleset's full statistics payload.
    Keeping this response compact also matters because the dashboard waits for
    these lookups before it can draw each online player card.
    """
    del request, background_task
    ...
    try:
        ruleset = GameMode.from_int_extra(ruleset_id) if ruleset_id is not None else None
    ...
    transformed_users: list[UserDict] = []
    for user_id in requested_ids:
        user = users_by_id.get(user_id)
        if user is None:
            continue

        transformed = await UserModel.transform(user, includes=User.CARD_INCLUDES)
        if ruleset is not None:
            statistics = await UserModel.statistics(session, user, ruleset=ruleset)
            transformed["global_rank"] = {
                "rank": statistics.get("global_rank") if statistics is not None else None,
